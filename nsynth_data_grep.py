...
GROUPS = 10000 # Run model on groups of GROUPS
...
od_data = './nsynth_files/nsynth_data/'
    ...
    for i, filename in enumerate(os.listdir(in_dir)):
        ...
        for item in Ins_family:
            for pitch in pitch_range:
                # print("Looking for matching File... ")
                ...
                # Remove catching swapped pitch and velocity values
                    if velocity in filename:
                        count = count + 1
                        print(" Classifiable File FOUND: " + filename + '\n')
                        graph_spectrogram((in_dir + filename), i, item, pitch, out_dir, in_dir)
...
def graph_spectrogram_2(wav_file, index, instrument, pitch, out_dir, in_dir):
    ...
    pylab.figure(num=None, figsize=(0.72, 0.72))
    pylab.subplot(111)
    pylab.axis('off')
    pylab.box(False)
    spec = librosa.feature.melspectrogram(y=clip, sr=sample_rate)
    ...
    pylab.savefig(((new_path + file_name) + '.png'), dpi=400, bbox_inches='tight', pad_inches=0)
    pylab.close()
